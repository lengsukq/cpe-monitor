...
export default function CellSnapshotCard({
  ...
  return (
    <Card className="card-hover overflow-hidden border-brand/15 bg-[radial-gradient(circle_at_top_right,color-mix(in_oklch,var(--brand),transparent_84%),transparent_42%),linear-gradient(135deg,var(--card),var(--card))] py-4 sm:py-5">
      <CardHeader className="flex flex-col gap-2 px-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-6">
        <div>
          ...
          <p className="mt-1 hidden text-sm text-muted-foreground sm:block">实时蜂窝注册信息与 CPE 身份</p>
        </div>
