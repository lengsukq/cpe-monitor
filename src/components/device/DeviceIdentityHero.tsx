...
export default function DeviceIdentityHero({ info, cell }: DeviceIdentityHeroProps) {
  ...
  return (
    <motion.section
      className="app-panel relative overflow-hidden px-6 py-7 lg:px-8"
    ...
    >
      ...
      <div className="relative grid gap-6 xl:grid-cols-[minmax(0,.85fr)_minmax(0,1.15fr)] xl:items-end">
        ...
        <div className="fluid-card-grid min-w-0 gap-3 [--fluid-card-min:11rem]">
          {[{ icon: <Phone className="h-4 w-4" />, label: 'SIM 手机号', value: info.Msisdn || '未返回', mono: false },
            { icon: <Radio className="h-4 w-4" />, label: '当前小区', value: cell?.cellId || '未返回', mono: true },
            { icon: <ShieldCheck className="h-4 w-4" />, label: 'ICCID', value: info.Iccid || '未返回', mono: true },
          ].map((tile, index) => (
            <motion.div
              key={tile.label}
              initial={reduce ? undefined : { opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.35, delay: 0.14 + index * 0.07, ease: 'easeOut' }}
            >
              <IdentityTile icon={tile.icon} label={tile.label} value={tile.value} mono={tile.mono} />
            </motion.div>
          ))}
        </div>
      </div>
